# print("Hello World!")
# print("I DO need change (life) career, transition activate!")
# print("Paris 2024, so amazing!")
# print("COYG")
#
# print(2024 + 2025 - 3333)
# print("hello world".upper())
#
# print(len("Hello World!"))
# print(len("01001010001010100101010010101011101101101010100101"))
# #
# print("Hello World"[1])
# print("Hello World"[8])
#
print("the quick brown fox".upper())
print("THE QUICK BROWN FOX".lower())
print("the quick brown fox".capitalize())
print("The Quick Brown fox fox fox".count("fox"))
print("The Quick Brown fox fox fox".replace("fox", "python"))